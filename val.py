...
def run(data,
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        for si, pred in enumerate(out):
            ...
            # Save/log
            # if True:
            #     save_one_txt(predn, save_conf, shape, file='yoloair/yoloair-main/runs/val1/'+(path.stem + '.txt'))
            # if save_json:
            #     save_one_json(predn, jdict, path, class_map)  # append to COCO-JSON dictionary
            # callbacks.run('on_val_image_end', pred, predn, path, names, im[si])
...
def parse_opt():
    ...
    parser.add_argument('--data', type=str, default=ROOT / 'data/steel.yaml', help='dataset.yaml path')
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'runs/train/exp310/weights/best.pt', help='model.pt path(s)')
