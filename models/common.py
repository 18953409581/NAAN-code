...
class Bottleneck38(nn.Module):
    # Standard bottleneck
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, shortcut, groups, expansion
        super().__init__()
        c_ = int(c1//2)  # hidden channels
        self.cv1 = Conv(c_, c1, 1, 1)
    ...
    def forward(self, x):
        ...
        x = x1+x2
        return identy + self.cv2(self.cv1(x)) if self.add else self.cv2(self.cv1(x))
...
class C38(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        c3 = c1 //2
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c3, c_, 1, 1)
        self.cv3 = Conv(2 * c_, c2, 1)  # act=FReLU(c2)

        self.m = nn.Sequential(*(Bottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n)))

    def forward(self, x):
        y1,y2 = self.channel_shuffle(x)

        y = y1 +y2
        return self.cv3(torch.cat((self.m(self.cv1(x)), self.cv2(y)), dim=1))
...
class C39(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        c_ = int(c1 * 1.5)  # hidden channels
        # self.cv1 = Conv(c1, c_, 1, 1)
        # self.cv2 = Conv(int(1.5 * c1), c_, 1, 1)
        self.cv3 = Conv(c_, c2, 1)  # act=FReLU(c2)
        self.m = nn.Sequential(*(Bottleneck38(c_, c_, shortcut, g, e=1.0) for _ in range(n)))

    def forward(self, x):
        y1, y2 = self.channel_shuffle(x)
        y11, y12 = self.channel_shuffle(y1)
        y13, y14 = self.channel_shuffle(y2)
        x1 = y11 + y12
        x2 = y11 + y13
        x3 = y11 + y14
        x4 = y12 + y13
        x5 = y12 + y14
        x6 = y13 + y14
        t = torch.cat((x1, x2, x3, x4, x5, x6), dim=1)

        return self.cv3(self.m(self.cv1(t)))
...
class TransformerLayerS(nn.Module):
    ...
    def __init__(self, c1,c2, num_heads):
        ...
        self.fc1 = nn.Linear(c2, c2, bias=False)
        self.fc2 = nn.Linear(c2, c2, bias=False)

    def forward(self, x):
        #print(x.shape)
        x1, x2, x3, x4 = x[..., ::2, ::2], x[..., 1::2, ::2], x[..., ::2, 1::2], x[..., 1::2, 1::2]
        if self.conv is not None:
            x1 = self.conv(x1)
        b1, _, w1, h1 = x1.shape
        if self.conv is not None:
        ...
        p2 = x2.flatten(2).permute(2, 0, 1)
        p3 = x3.flatten(2).permute(2, 0, 1)
        p4 = x4.flatten(2).permute(2, 0, 1)
        x1 = p1 + self.linear(p1)
        x2 = p2 + self.linear(p2)
        x3 = p3 + self.linear(p3)
        x4 = p4 + self.linear(p4)
        y1 = self.ma(self.q(x1), self.k(x2), self.v(x1))[0].permute(1,2,0).reshape(b1, self.c2, w1, h1)
        # y2 = self.ma(self.q(x4), self.k(x2), self.v(x3))[0].permute(1,2,0).reshape(b2, self.c2, w2, h2)
        # y3 = self.ma(self.q(x4), self.k(x3), self.v(x3))[0].permute(1,2,0).reshape(b3, self.c2, w3, h3)
        y4 = self.ma(self.q(x4), self.k(x3), self.v(x4))[0].permute(1,2,0).reshape(b4, self.c2, w4, h4)


        x[..., ::2, ::2], x[..., 1::2, 1::2] = y1,y4
        return x
class TransformerBlockSS(nn.Module):
    # Vision Transformer https://arxiv.org/abs/2010.11929
    def __init__(self, c1, c2, num_heads, num_layers):
        super().__init__()
        self.conv = None
        if c1 != c2:
            self.conv = Conv(c1, c2)
        self.linear = nn.Linear(c2, c2)  # learnable position embedding
        self.trs = nn.Sequential(*(TransformerLayerS(c2,c2, num_heads) for _ in range(num_layers)))
...
class BottleneckTR(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, shortcut, groups, expansion
        ...
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c_, c2, 3, 1, g=g)
        self.tr = TransformerBlockSS(c_, c_, 4, 1)
        self.add = shortcut and c1 == c2

    def forward(self, x):

        return x + self.cv2(self.tr(self.cv1(x))) if self.add else self.cv2(self.cv1(x))
class C3TRCS(C3):
    # C3 module with TransformerBlock()
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)
        self.m = TransformerBlockSS(c_, c_, 4, n)
...
class C3RR(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(2 * c_, c2, 1)  # act=FReLU(c2)
        self.m = nn.Sequential(*(BottleneckTR(c_, c_, shortcut, g, e=1.0) for _ in range(n)))
...
class C37(nn.Module):
    ...
    def forward(self, x):
        return self.cv3(torch.cat((self.m(self.cv1(x)), self.tr(self.cv2(x))), 1))
...
class bingxing(nn.Module):
    ...
    def forward(self, x):
            ...
            x4 = self.con1(self.act(self.bn(x4)))
            return x4
...
class Concat10(nn.Module):
    # Concatenate a list of tensors along dimension
    def __init__(self, dimension=1):
        super().__init__()
        self.d = dimension
        self.conv1 = Conv(512,512,3,2)
        self.bn = nn.BatchNorm2d(512)

    def forward(self, x):

        return self.conv1(self.bn(torch.cat(x, self.d)))
...
class CoordAtt(nn.Module):
    def __init__(self, inp, oup, reduction=32):
        ...
        self.conv2to1 = nn.Conv2d(oup, oup, kernel_size=1, stride=1, padding=0)
...
class recuir30(nn.Module):
    # Concatenate a list of tensors along dimension
    def __init__(self, c1,c2,dimension=1,drop_path = 0.15):
        super().__init__()
        self.d = dimension
        self.up = nn.UpsamplingNearest2d(scale_factor=2)
        self.conv01 = Conv(c2, c1)
        self.conv1 = ConV2(c1, c2)
        self.conv02 = nn.Conv2d(c2, c1, kernel_size=1, stride=1, padding=0)
        self.cbam = CoordAtt(c1, c1)
        self.conv031 = Conv(c1,c2,1,1)
        self.conv032 = Conv(c2, c2, 3, 2)
        self.conv40 = Conv(c2 * 2, c2 *2, 1, 1)
        self.bn = nn.BatchNorm2d(c2 * 2)  # applied to cat(cv2, cv3)
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
    ...
    def forward(self, x):
            ...
            x4 = torch.cat([x3,identy],1)
            x4 = self.conv40(self.bn(x4))

            return x4

class recuir40(nn.Module):
    # Concatenate a list of tensors along dimension
    def __init__(self, c1,c2,dimension=1,drop_path = 0.15):
        ...
        self.up = nn.UpsamplingNearest2d(scale_factor=2)
        self.conv01 = Conv(c2, c1)
        self.conv40 = Conv(c2 * 2, c2 , 1, 1)
        self.bn = nn.BatchNorm2d(c2 * 2)  # applied to cat(cv2, cv3)
        self.conv031 = Conv(c1, c2, 1, 1)
        self.conv032 = Conv(c2, c2, 3, 2)
        self.cc = C3Ghost(c1,c1)
...
class recuir50(nn.Module):
    # Concatenate a list of tensors along dimension
    def __init__(self, c1,c2,dimension=1):
        super().__init__()
        self.bn = nn.BatchNorm2d(c2 * 2)  # applied to cat(cv2, cv3)
        self.cc = C3Ghost(c2,c2)
...
class CBAM1(nn.Module):
    def __init__(self, c1 ,c2):
        ...
        self.channel_attention = Channel_Att1(c1)
        #self.spatial_attention = CoordAtt1(c1,c2)

    def forward(self, x,x2):
        out = self.channel_attention(x) * x2
...
class C3GTrans(nn.Module):
    # C3 module with GhostBottleneck()
    def __init__(self, c1, n=1, shortcut=True, g=1, e=0.5):
        super().__init__()
        c_ = int(c1 * e)  # hidden channels
        self.m = nn.Sequential(*(GhostBottleneck(c_, c_) for _ in range(n)))
        self.trans = C3TRCS(c1,c1)
...
def forward(self, x):
        ...
        y2 = self.trans(x)
        return self.bn(torch.cat([y1,y2],1))
...
class Conv11(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        super().__init__()
        self.conv = nn.Conv2d(c2, c2, k, s, autopad(k, p), groups=g, bias=False)
        self.bn = nn.BatchNorm2d(c2)
        self.act = nn.SiLU() if act is True else (act if isinstance(act, nn.Module) else nn.Identity())
        self.c3 = C3Ghost(c1,c1)
...
class C3TRC(C3):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)
        self.m = TransformerBlockSplitKernnel(c_, c_, 4, n)
...
class C3S(C3):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        ...
        self.epsilon = 0.0001
        self.m = TSK(c_, c_, 4, n)
        self.c = c_

    def forward(self, x):
        w = self.w
        weight = w / (torch.sum(w, dim=0) + self.epsilon)  # 将权重进行归一�?        w1 = int(weight[0] * self.c)
        w2 = int(weight[1] * self.c)
        w3 = int(weight[2] * self.c)
        w4 = self.c - w1 -w2 -w3
        w3 = w3 + w4
        return self.cv3(torch.cat((self.m(self.cv1(x)), self.cv2(x)), dim=1))


class TransformerBlockSplitKernnel(nn.Module):
    # Vision Transformer https://arxiv.org/abs/2010.11929
    def __init__(self, c1, c2, num_heads, num_layers):
        super().__init__()
        self.conv = None
        ...
        self.linear = nn.Linear(c2, c2)  # learnable position embedding
        self.tr = nn.Sequential(*(TransformerLayer(c2, num_heads) for _ in range(num_layers)))
        self.conv1 = Conv(int(c2*0.5), c2)
        self.conv2 = Conv(c2,int(c2 * 0.5))
        self.c2 = c2
    ...
    def forward(self, x):
        ...
        b1, _, w1, h1 = x.shape
        identy = x
        x1 = x[:, 0:int(self.c2*0.5), :, :]
        x2 = x[:, int(self.c2*0.5):self.c2, :, :]
        x1 = self.conv1(x1)
        x2 = self.conv1(x2)
        p1 = x1.flatten(2).permute(2, 0, 1)
        p2 = x2.flatten(2).permute(2, 0, 1)
        y1 = self.tr(p1 + self.linear(p1)).permute(1,2,0).reshape(b1, self.c2, w1, h1)
        y2 = self.tr(p2 + self.linear(p2)).permute(1, 2, 0).reshape(b1, self.c2, w1, h1)
        y1 = self.conv2(y1)
        y2 = self.conv2(y2)
        x[:, ::2, :, :] = y1
        x[:, 1::2, :, :] =y2
...
class TSK(nn.Module):
    ...
    def forward(self, x):
        ...
        b1, _, w1, h1 = x.shape
        w = self.w
        weight = w / (torch.sum(w, dim=0) + self.epsilon)  # 将权重进行归一�?
...
class BiFPN_Add2(nn.Module):
    def __init__(self, c1, c2):
        ...
        # 设置可学习参�?nn.Parameter的作用是：将一个不可训练的类型Tensor转换成可以训练的类型parameter
        # 并且会向宿主模型注册该参�?成为其一部分 即model.parameters()会包含这个parameter
        # 从而在参数优化的时候可以自动一起优�?        self.w = nn.Parameter(torch.ones(2, dtype=torch.float32), requires_grad=True)
        self.epsilon = 0.0001
...
class GB(nn.Module):
    ...
    def __init__(self, c1, c2, k=3, s=1):  # ch_in, ch_out, kernel, stride
        super().__init__()
        c_ = c2 // 2
        self.conv = nn.Sequential(Conv(c1, c_, 1, 1),  # pw
...
class C3GhostT(C3):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(GB(c_, c_) for _ in range(n)))

class segcat(nn.Module):
    def __init__(self, dimension=1):
        super().__init__()
        self.d = dimension
        self.conv1 = Conv(512, 512, 3, 2)
