...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv,Conv10,Conv20, GhostConv, Bottleneck,Bottleneck38, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus, CrossConv,
                 BottleneckCSP, C3,C31,C7,C37,C38, C3TR,C3TRCS,C3VB,C3TRC,C3RR,BottleneckTR,  C3SPP, C3Ghost,C3GhostT, C3HB, C3RFEM, MultiSEAM, SEAM, C3STR, MobileOneBlock]:
            c1, c2 = ch[f], args[0]
            if c2 != no:  # if not output
                c2 = make_divisible(c2 * gw, 8)

            args = [c1, c2, *args[1:]]
            if m in [BottleneckCSP, C3,C31,C7,C37,C38, C3TR,C3TRCS,C3TRC, C3VB, C3Ghost,C3GhostT, C3HB, C3RFEM, C3STR]:
                args.insert(2, n)  # number of repeats
                n = 1
        # add module research
        elif m in [CARAFE, SPPCSPC, SPPFCSPC, RepConv, BoT3, CA,C3RR, CBAM,BottleneckTR, NAMAttention, GAMAttention, Involution, Stem, ResCSPC, ResCSPB, \
                   ResXCSPB, ResXCSPC, BottleneckCSPB, BottleneckCSPC,C3GTrans,
